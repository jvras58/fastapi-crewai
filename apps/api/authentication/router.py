"""Authentication router module."""
from typing import Annotated

from fastapi import APIRouter, Depends, HTTPException
from fastapi.security import OAuth2PasswordRequestForm

from apps.api.authentication.controller import execute_user_login
from apps.api.authentication.schemas import AccessToken
from apps.database.session import Session, get_session
from apps.utils.exceptions import IncorrectCredentialException

router = APIRouter()

SessionDep = Annotated[Session, Depends(get_session)]
OAuth2Form = Annotated[OAuth2PasswordRequestForm, Depends()]


@router.post('/token', response_model=AccessToken)
def login_for_access_token(
    form_data: OAuth2Form, db_session: SessionDep
) -> dict:
    """Endpoint to obtain JWT token."""
    try:
        return execute_user_login(
            db_session, form_data.username, form_data.password
        )
    except IncorrectCredentialException as ex:
        raise HTTPException(status_code=400, detail=ex.args[0]) from ex
