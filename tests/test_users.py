...
from apps.core.api.user.schemas import UserPublic
...
def test_read_users(client, token):
    with patch(
        'apps.core.api.user.router.validate_transaction_access'
...
def test_get_user_transactions(
    ...
    # SETUP
    with patch(
        'apps.core.api.assignment.router.validate_transaction_access'
    ) as mocked_access_validation:
        response = client.post(
            '/assignment/',
            headers={'Authorization': f'Bearer {token}'},
