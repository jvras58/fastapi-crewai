...
def test_upload_document_success(mock_doc_controller, client, token, user):
    ...
    with patch(
        "apps.ia.api.documents.router.validate_transaction_access"
...
def test_upload_document_invalid_data(client, token):
    ...
    with patch("apps.ia.api.documents.router.validate_transaction_access"):
        response = client.post(
            "/ia/documents",
...
def test_list_documents(client, token, multiple_documents):
    ...
    with patch(
        ...
        response = client.get(
            "/ia/documents", headers={"Authorization": f"Bearer {token}"}
        )
...
def test_list_documents_pagination(client, token, multiple_documents):
    ...
    with patch(
        ...
        data = response.json()
        assert data["page"] == 2
        mocked_access_validation.assert_called()
...
def test_search_knowledge_base_no_query(client, token):
    ...
    with patch("apps.ia.api.documents.router.validate_transaction_access"):
        response = client.get(
            "/ia/search", headers={"Authorization": f"Bearer {token}"}
        )
        assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
...
def test_search_knowledge_base_k_limit(client, token):
    ...
    with (
        ...
        patch("apps.ia.api.documents.router.validate_transaction_access"),
    ):
        response = client.get(
            '/ia/search?q=test&k=25',
            headers={'Authorization': f'Bearer {token}'},
        )
...
def test_search_knowledge_base_empty_results(client, token):
    ...
    with (
        patch("apps.ia.api.documents.controller.RAGService") as mock_rag_service,
        patch("apps.ia.api.documents.router.validate_transaction_access"),
    ):
        mock_rag = Mock()
...
def test_document_api_error_handling(client, token):
    ...
    with (
        ...
        response = client.get(
            '/ia/search?q=test', headers={'Authorization': f'Bearer {token}'}
        )
