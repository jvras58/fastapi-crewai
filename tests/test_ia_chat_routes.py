...
def test_send_chat_message_new_conversation(client, token, user):
    """Test sending a chat message to create a new conversation."""
    with (
        patch(
            "apps.ia.api.chat.router.validate_transaction_access"
        ) as mocked_access_validation,
        patch("apps.ia.api.chat.controller.ConversationAgent") as mock_agent_class,
    ):
        mock_agent = Mock()
        ...
        mocked_access_validation.assert_called_once()


def test_send_chat_message_existing_conversation(client, token, user, conversation):
...
def test_send_chat_message_invalid_data(client, token):
    ...
    with patch("apps.ia.api.chat.router.validate_transaction_access"):
        response = client.post(
            "/ia/chat",
            json={"message": ""},
            headers={"Authorization": f"Bearer {token}"},
        ...
        response = client.post(
            "/ia/chat", json={}, headers={"Authorization": f"Bearer {token}"}
        )
        assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
...
def test_send_chat_message_invalid_conversation_id(client, token):
    ...
    with patch(
        ...
        chat_data = {"message": "Olá, preciso de ajuda", "conversation_id": 99999}
...
def test_list_conversations(client, token, multiple_conversations):
    ...
    with patch(
        ...
        response = client.get(
            "/ia/conversations", headers={"Authorization": f"Bearer {token}"}
        )
...
def test_get_nonexistent_conversation(client, token):
    ...
    with patch(
        ...
        response = client.get(
            "/ia/conversations/99999", headers={"Authorization": f"Bearer {token}"}
        )
...
def test_update_conversation_unauthorized(client, conversation):
    ...
    update_data = {'title': 'Não autorizado'}

    response = client.put(
        f'/ia/conversations/{conversation.id}', json=update_data
    )
    assert response.status_code == status.HTTP_401_UNAUTHORIZED
...
def test_full_conversation_flow(client, token, user):
    ...
    with (
        ...
        update_data = {"title": "Conversa Finalizada", "status": "archived"}
        ...
        assert mocked_access_validation.call_count == 5
